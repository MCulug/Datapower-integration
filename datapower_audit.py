...
<dp:get-file name="%s"/>
 ...
 host="https://your_target_host_address:5550"
 AUTH = 'admin', 'XXXXXXX'                                                  #5550 is the default port of xml management interface in IBM datapower server which must be enabled before execution
...
if os.path.isfile('switch/audit-log'):                                        #Here is the switch mechanism. These two if statements will switch audit-log and audit-log.1 file
 ...
 if filecmp.cmp('audit_new', 'switch/audit-log') == False:
  print('files sizes are different. New audit-log file will be overriden')    #/switch and /audit directories must be created before running code. 
  ...
  file = open("switch/audit-log.1","w")
  file.close()

if os.path.isfile('switch/audit-log.1'):
 print ("Working with audit-log.1 file")
 fetch("audit:///audit-log.1")
 if filecmp.cmp('audit_new', 'switch/audit-log.1') == False:
  print('files sizes are different. New audit-log.1 file will be overriden')
  os.remove('switch/audit-log.1')
  shutil.move('audit_new', 'switch/audit_new')
  os.rename('switch/audit_new', 'switch/audit-log.1')
 else:
  time = str(time.strftime("%d %B %Y %H:%M:%S"))
  ...
  os.rename('Audit/audit-log.1', 'Audit/'+time)
